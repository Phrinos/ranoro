...
Sistema de gestión para un **taller mecánico en Aguascalientes, México** (Ranoro). Next.js 16 (App Router) + Firebase. Este archivo es la capa de **dominio**; las convenciones generales (idioma, estilo, árbol-siempre-verde, patrones Firestore/Next, secretos) viven en las convenciones globales y NO se repiten aquí.
...
- **Tipos centrales:** `src/types/index.ts` (Vehicle, ServiceRecord, ServiceItem, Payment, SaleReceipt, Driver, InventoryItem, Appointment, AuditLog, User, AppRole…). Enums centralizados aquí (PaymentMethod, ServiceSubStatus, AppointmentStatus…). No dupliques uniones: si necesitas extender una, hazlo en este archivo.
...
## IA y WhatsApp — retirados (17-sep-2026)

Por decisión del dueño este repo **no usa Gemini/IA ni tiene módulo de WhatsApp** (bot, bandeja, envíos, recordatorios). No reintroducirlos sin pedido explícito. Los datos históricos `whatsapp-*` siguen en Firestore, cerrados por deny-by-default; el contacto público son enlaces `wa.me`. `settings/whatsapp-agent` sigue **excluido** del wildcard de `settings` en `firestore.rules`: guarda credenciales del bridge en producción; no quitar esa exclusión. Acta (sesión multi-repo): `../tigrefeliz/docs/historial/004-gemini-estandar-y-retiro-ia-multi-repo.md`.
