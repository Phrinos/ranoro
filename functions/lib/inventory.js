...
exports.adjustStock = exports.onPurchaseUpdated = exports.onPurchaseCreated = exports.onStockExit = void 0;
 ...
 */
exports.onStockExit = (0, firestore_1.onDocumentUpdated)("serviceRecords/{serviceId}", async (event) => {
 ...
 */
const processStockEntry = async (purchaseSnap, purchaseId) => {
    const db = admin.firestore(); // Obtener la instancia de db aquí
 ...
 */
exports.onPurchaseUpdated = (0, firestore_1.onDocumentUpdated)("purchases/{purchaseId}", async (event) => {
    if (!event.data) {
        return;
    }
    const dataBefore = event.data.before.data();
...
exports.adjustStock = (0, https_1.onCall)(async (request) => {
    ...
    if (!itemId || typeof newQuantity !== 'number' || newQuantity < 0 || !reason) {
        throw new https_1.HttpsError("invalid-argument", "Faltan o son inválidos los parámetros (itemId, newQuantity, reason).");
    }
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            if (!itemDoc.exists) {
                throw new https_1.HttpsError("not-found", `El item con ID ${itemId} no existe.`);
            }
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError("internal", "Ocurrió un error al ajustar el inventario.");
    }
});
