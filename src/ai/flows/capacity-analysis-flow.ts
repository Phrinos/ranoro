...
// Client-side helper to calculate duration. The AI will use this pattern in its reasoning.
const calculateDuration = (start: string, end?: string): number => {
    if (!end) return 1; // Default to 1 hour if no end time
    try {
        const startDate = parseISO(start);
        const endDate = parseISO(end);
        const hours = differenceInHours(endDate, startDate);
        return Math.max(1, Math.round(hours)); // Return at least 1 hour
    } catch {
        return 1;
    }
};
...
**Instructions:**
1.  **Analyze Historical Data:** Review the \`serviceHistory\`. For each past service, calculate the duration in hours between \`serviceDate\` and \`deliveryDateTime\`. Learn the typical duration for different types of job descriptions. For example, a "Cambio de aceite" might consistently take 1 hour, while a "reparación de motor" might take 8-16 hours.
...
{{#each serviceHistory}}
- Description: "{{description}}", Duration: ~{{calculateDuration serviceDate deliveryDateTime}} hours
{{/each}}
...
const capacityAnalysisFlow = ai.defineFlow(
  ...
  async (input) => {
    // The prompt is given the helper function context but doesn't execute it.
    // The AI is expected to learn the pattern.
    const { output } = await capacityAnalysisPrompt.generate({
        input,
        config: { temperature: 0.2 },
        // Providing the helper function to the prompt context.
        // NOTE: The AI cannot execute this function. It's for pattern matching.
        context: { calculateDuration },
    });
