        ...
        <>
            <div className="bg-primary text-primary-foreground rounded-lg p-6 mb-6">
             ...
             <div className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                <p className="font-semibold text-lg">Filtrar por Mes</p>
                <div className="flex items-center gap-2 flex-wrap justify-start sm:justify-end">
                    ...
                    <Popover><PopoverTrigger asChild><Button variant={"outline"} className={cn("w-full sm:w-[240px] justify-start text-left font-normal", !dateRange && "text-muted-foreground")}><CalendarIcon className="mr-2 h-4 w-4" />{dateRange?.from ? (dateRange.to ? (`${format(dateRange.from, "LLL, y", { locale: es })}`) : format(dateRange.from, "LLL dd, y", { locale: es })) : (<span>Seleccione mes</span>)}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="end"><Calendar initialFocus mode="single" defaultMonth={dateRange?.from} selected={dateRange?.from} onSelect={(date) => date && setDateRange({from: startOfMonth(date), to: endOfMonth(date)})} numberOfMonths={1} locale={es} /></PopoverContent></Popover>
                </div>
            </div>
