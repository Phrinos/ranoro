...
import { Car, Gauge, Edit, User as UserIcon, CalendarDays, History, StickyNote, Link as LinkIcon } from 'lucide-react';
...
export function DetailsTabContent({ vehicle, drivers, onEdit }: DetailsTabContentProps) {
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 text-sm">
            ...
            <div className="space-y-1">
             ...
             <div className="space-y-1">
                <p className="font-medium text-muted-foreground flex items-center gap-2"><UserIcon className="h-4 w-4" />Conductor Asignado</p>
                {assignedDriver ? (
                    <Button variant="link" asChild className="p-0 h-auto font-semibold text-base">
                        <Link href={`/conductores/${assignedDriver.id}`}>
                            {assignedDriver.name} <LinkIcon className="ml-2 h-4 w-4"/>
                        </Link>
                    </Button>
                ) : (
                    <p className="text-base text-muted-foreground">Sin conductor asignado</p>
                )}
            </div>
