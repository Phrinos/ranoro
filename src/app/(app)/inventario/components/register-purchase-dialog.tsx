...
export function RegisterPurchaseDialog({
  ...
  const subtotal = useMemo(() => {
    return (items || []).reduce((acc: number, item: any) => acc + (item.quantity || 0) * (item.purchasePrice || 0), 0);
  }, [items]);
                    ...
                    return (
                      <div key={field.id} className="grid grid-cols-12 gap-x-2 rounded-md border p-2">
                        ...
                        <div className="col-span-2 flex items-center">
                          <p className="text-sm">
                            Total:{" "}
                            {CURRENCY_FORMATTER.format((items?.[index]?.quantity || 0) * (items?.[index]?.purchasePrice || 0))}
                          </p>
                        </div>
