...
const ProductosContent = ({ inventoryItems, onPrint }: { inventoryItems: InventoryItem[], onPrint: (items: InventoryItem[]) => void }) => {
  ...
  const customSortedItems = React.useMemo(() => {
    const items = [...tableManager.fullFilteredData];
    if (tableManager.sortOption === 'updatedAt_asc') {
    ...
    // Handle other sorts if necessary, or rely on tableManager's default sort for others.
    return items;
            ...
            <Table>
              <TableHeader className="bg-black text-white">
                <TableRow>
                    ...
                    <TableHead className="text-white">Últ. Modificación</TableHead>
                    <TableHead className="text-right text-white">Precio Compra</TableHead>
                ...
                {customSortedItems.map(item => {
                    ...
                    return (
                        <TableRow key={item.id} onClick={() => router.push(`/inventario/${item.id}`)} className="cursor-pointer hover:bg-muted/50">
                            ...
                            <TableCell>{updatedAt && isValid(updatedAt) ? format(updatedAt, 'dd/MM/yy, HH:mm', { locale: es }) : 'N/A'}</TableCell>
