...
import { SupplierDialog } from '@/app/(app)/inventario/compras/components/supplier-dialog';
...
import { PayableAccountDialog } from '@/app/(app)/inventario/compras/components/payable-account-dialog';
                    ...
                    <TableBody>
                      {accountsPayable.length > 0 ? accountsPayable.map(acc => (
                        <TableRow key={acc.id}>
                            ...
                            <TableCell className="text-right">{formatCurrency(acc.totalAmount ?? 0)}</TableCell>
