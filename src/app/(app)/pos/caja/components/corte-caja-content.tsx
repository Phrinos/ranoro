...
export const CorteDiaContent = React.forwardRef<HTMLDivElement, CorteCajaContentProps>(({ reportData, date, transactions, previewWorkshopInfo }, ref) => {
  ...
  const finalBalance = reportData.dailyCashIn - reportData.dailyCashOut;
  ...
  return (
    ...
    >
      ...
      {renderLine("(+) Entradas:", formatCurrency(reportData.dailyCashIn), false, false, "text-green-700")}
      {renderLine("(-) Salidas:", formatCurrency(reportData.dailyCashOut), false, false, "text-red-700")}
      {renderDashedLine()}
      {renderLine("SALDO FINAL DEL DÍA:", formatCurrency(finalBalance), true, true)}
