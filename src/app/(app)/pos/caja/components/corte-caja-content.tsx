...
function TransactionsList({ transactions, onDelete }: { transactions: CashDrawerTransaction[], onDelete: (id: string) => void }) {
    ...
    return (
        <Table>
            <TableBody>
                {transactions.map(t => {
                    const description = `Se eliminará la transacción por ${formatCurrency(t.amount)}. Esta acción no se puede deshacer.`;
                    return (
                        <TableRow key={t.id}>
                            <TableCell>
...
export default function CorteCajaContent({ allSales, allServices, allCashTransactions, initialCashBalance }: CorteCajaContentProps) {
  ...
  const cajaSummaryData = useMemo(() => {
    ...
    const cashFromSales = salesInRange
        .flatMap(s => s.payments || (s.paymentMethod?.includes('Efectivo') ? [{ method: 'Efectivo', amount: s.totalAmount }] : []))
        .filter(p => p.method === 'Efectivo')
        .reduce((sum, p) => sum + (p.amount || 0), 0);
        
    const cashFromServices = servicesInRange
        .flatMap(s => s.payments || (s.paymentMethod?.includes('Efectivo') ? [{ method: 'Efectivo', amount: s.totalCost }] : []))
        .filter(p => p.method === 'Efectivo')
        .reduce((sum, p) => sum + (p.amount || 0), 0);
    ...
    [...salesInRange, ...servicesInRange].forEach(op => {
      const payments = op.payments || (op.paymentMethod ? [{ method: op.paymentMethod, amount: 'totalAmount' in op ? op.totalAmount : (op.totalCost || 0) }] : []);
      payments.forEach(p => {
          salesByPaymentMethod[p.method] = (salesByPaymentMethod[p.method] || 0) + (p.amount || 0);
      });
    });
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center text-green-600"><span>(+) Efectivo de Operaciones:</span> <span className="font-medium">{formatCurrency(cajaSummaryData.totalCashSales)}</span></div>
