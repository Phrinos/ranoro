                ...
                {serviceFields.map((serviceField, serviceIndex) => (
                    <Card key={serviceField.id} className="p-4 bg-muted/30">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <FormField control={form.control} name={`services.${serviceIndex}.serviceName`} render={({ field }) => ( <FormItem><FormLabel>Nombre Servicio</FormLabel><FormControl><Input placeholder="Afinación Mayor" {...field} onChange={(e) => field.onChange(capitalizeWords(e.target.value))} /></FormControl><FormMessage/></FormItem> )}/>
        ...
        <div className="space-y-3">
             {fields.map((supplyField, supplyIndex) => (
                <div key={supplyField.id} className="grid grid-cols-1 md:grid-cols-10 gap-2 items-end">
                    <FormField control={control} name={`services.${serviceIndex}.supplies.${supplyIndex}.name`} render={({ field }) => ( <FormItem className="col-span-10 md:col-span-3"><FormLabel className="text-xs">Nombre</FormLabel><FormControl><Input placeholder="Aceite Sintético 5W-30" {...field} onChange={(e) => field.onChange(capitalizeWords(e.target.value))} /></FormControl><FormMessage/></FormItem> )}/>
