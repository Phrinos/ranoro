      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField control={form.control} name="make" render={({ field }) => ( <FormItem><FormLabel>Marca</FormLabel><FormControl><Input placeholder="Ej: Nissan" {...field} onChange={(e) => field.onChange(capitalizeWords(e.target.value))} className="bg-white" /></FormControl><FormMessage /></FormItem> )}/>
            <FormField control={form.control} name="model" render={({ field }) => ( <FormItem><FormLabel>Modelo</FormLabel><FormControl><Input placeholder="Ej: Versa" {...field} onChange={(e) => field.onChange(capitalizeWords(e.target.value))} className="bg-white" /></FormControl><FormMessage /></FormItem> )}/>
            ...
            <CardContent className="space-y-4">
                {serviceFields.map((serviceField, serviceIndex) => (
                    <Card key={serviceField.id} className="p-4 bg-muted/30">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"><FormField control={form.control} name={`services.${serviceIndex}.serviceName`} render={({ field }) => ( <FormItem><FormLabel>Nombre Servicio</FormLabel><FormControl><Input placeholder="Afinación Mayor" {...field} onChange={(e) => field.onChange(capitalizeWords(e.target.value))} className="bg-white" /></FormControl><FormMessage/></FormItem> )}/>
                            <FormField control={form.control} name={`services.${serviceIndex}.customerPrice`} render={({ field }) => ( <FormItem><FormLabel>Precio Cliente</FormLabel><FormControl><Input type="number" placeholder="1999.00" {...field} value={field.value ?? ''} className="bg-white" /></FormControl><FormMessage/></FormItem> )}/>
                            <FormField control={form.control} name={`services.${serviceIndex}.estimatedTimeHours`} render={({ field }) => ( <FormItem><FormLabel>Horas Est.</FormLabel><FormControl><Input type="number" step="0.1" placeholder="2.5" {...field} value={field.value ?? ''} className="bg-white" /></FormControl><FormMessage/></FormItem> )}/>
                            <FormField control={form.control} name={`services.${serviceIndex}.description`} render={({ field }) => ( <FormItem className="md:col-span-3"><FormLabel>Descripción</FormLabel><FormControl><Textarea rows={2} placeholder="Incluye cambio de aceite, filtros y bujías." {...field} className="bg-white" /></FormControl><FormMessage/></FormItem> )}/>
                        </div>
