  ...
  const handleSaveService = async (values: ServiceFormValues): Promise<ServiceRecord | void> => {
    ...
    try {
        ...
        if (!isEditMode) {
            ...
            if (publicService) {
                setRecordForPreview(publicService);
                redirectUrl.current = `/servicios?tab=cotizaciones`;
                setIsShareDialogOpen(true);
            } else {
...
```
    - `src/app/(app)/servicios/components/ServiceForm.tsx`: El `onSaveSuccess` prop ahora llama a `handleShowShareDialog`.
    - `src/lib/services/service.service.ts`: La función `saveService` ahora devuelve el registro guardado, y se añade la función `createOrUpdatePublicService` para manejar la creación del documento público.
    - `src/lib/public-document.ts`: He refactorizado `savePublicDocument` para que sea más robusto y se pueda llamar desde el lado del servidor.
    - `src/app/(app)/servicios/[id]/page.tsx`: He ajustado la lógica para que, al crear una nueva cotización (`isEditMode` es `false`), se llame a `createOrUpdatePublicService` y luego se muestre el diálogo de compartir. Esto asegura que el `publicId` exista antes de intentar mostrar el diálogo.
- `src/app/(app)/servicios/components/ServiceForm.tsx` (Typo en la descripción, corregido)
- `src/lib/services/service.service.ts`:
    - `saveService`: Ahora devuelve el `ServiceRecord` completo después de guardarlo.
    - `createOrUpdatePublicService`: Nueva función para manejar la lógica de creación del documento público de forma aislada.
- `src/app/(app)/servicios/[id]/page.tsx`:
    - `handleSaveService`: Se ha modificado el flujo para nuevas cotizaciones. Ahora, después de guardar el servicio principal, se crea el documento público y solo entonces se llama a `handleShowShareDialog` con los datos actualizados, asegurando que el `publicId` esté presente.
- `src/lib/public-document.ts`: Refactorizado para mayor claridad y para ser usado desde el lado del servidor de forma segura.

Con estos cambios, el guardado de una nueva cotización ahora completa el proceso correctamente, mostrando el diálogo para compartir con un enlace válido.

<changes>
  <change>
    <file>src/app/(app)/servicios/[id]/page.tsx</file>
    <content><![CDATA[
// src/app/(app)/servicios/[id]/page.tsx
"use client";
