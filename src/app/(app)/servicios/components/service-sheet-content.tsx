        ...
        <main className="flex-grow">
          ...
          <section className="grid grid-cols-2 gap-4 mb-4 text-xs">
            <div className="border border-gray-400 p-3 rounded">
              <h3 className="font-bold text-sm mb-2 border-b-2 border-gray-400 pb-1">DATOS DEL CLIENTE</h3>
              <div className="space-y-1">
                <p><span className="font-semibold">Nombre:</span> <span className="text-sm font-medium">{vehicle?.ownerName}</span></p>
                <p><span className="font-semibold">Teléfono:</span> <span className="text-sm font-medium">{vehicle?.ownerPhone}</span></p>
                <p><span className="font-semibold">Email:</span> <span className="text-sm font-medium">{vehicle?.ownerEmail || 'N/A'}</span></p>
              </div>
            </div>
            <div className="border border-gray-400 p-3 rounded">
              <h3 className="font-bold text-sm mb-2 border-b-2 border-gray-400 pb-1">DATOS DEL VEHÍCULO</h3>
              <div className="space-y-1">
                <p><span className="font-semibold">Marca/Modelo:</span> <span className="text-sm font-medium">{vehicle?.make} {vehicle?.model}</span></p>
                <p><span className="font-semibold">Año:</span> <span className="text-sm font-medium">{vehicle?.year}</span></p>
                <p><span className="font-semibold">Placas:</span> <span className="text-sm font-bold">{vehicle?.licensePlate}</span></p>
                <p><span className="font-semibold">Color:</span> <span className="text-sm font-medium">{vehicle?.color || 'N/A'}</span></p>
                <p><span className="font-semibold">Kilometraje:</span> <span className="text-sm font-medium">{service.mileage?.toLocaleString('es-MX') || 'N/A'} km</span></p>
              </div>
            </div>
          </section>
          ...
          <section className="border border-gray-400 p-3 rounded mb-4">
              <h3 className="font-bold text-sm mb-2 border-b-2 border-gray-400 pb-1">SERVICIO SOLICITADO</h3>
              <p className="whitespace-pre-wrap min-h-[60px] text-base font-bold">{service.description}</p>
          </section>
          
          <section className="grid grid-cols-3 gap-4 mb-4 text-xs">
              <div className="border border-gray-400 p-3 rounded col-span-2">
                 <h3 className="font-bold text-sm mb-2 border-b-2 border-gray-400 pb-1">CONDICIONES DEL VEHÍCULO (AL RECIBIR)</h3>
                 <p className="whitespace-pre-wrap min-h-[60px]">{service.vehicleConditions || 'No especificado.'}</p>
              </div>
              <div className="border border-gray-400 p-3 rounded col-span-1 flex flex-col">
                  <h3 className="font-bold text-sm mb-2 border-b-2 border-gray-400 pb-1 text-center">NIVEL DE COMBUSTIBLE</h3>
          ...
          {/* Customer Items */}
          <section className="border border-gray-400 p-3 rounded mb-4">
              <h3 className="font-bold text-sm mb-2 border-b-2 border-gray-400 pb-1">INVENTARIO DE PERTENENCIAS</h3>
              <p className="whitespace-pre-wrap min-h-[40px]">{service.customerItems || 'No especificado.'}</p>
          </section>
        </main>
