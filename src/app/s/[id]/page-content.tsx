...
interface PublicServiceSheetClientViewProps {
  initialData: {
...
export function PublicServiceSheetClientView({ initialData }: PublicServiceSheetClientViewProps) {
  const params = useParams();
  const publicId = params.id as string;
  const { toast } = useToast();
  const serviceSheetRef = useRef<HTMLDivElement>(null);

  const [service, setService] = useState<ServiceRecord | null>(initialData.service);
  const [vehicle, setVehicle] = useState<Vehicle | null>(initialData.vehicle);
  const [workshopInfo, setWorkshopInfo] = useState<WorkshopInfo | null>(initialData.workshopInfo);
