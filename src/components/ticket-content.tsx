...
/** Ajustes de ticket (branding + tipografía) */
...
const defaultTicketSettings: Required<Pick<
  TicketSettings,
        ...
        <div className="py-0.5 space-y-1" style={{ fontSize: `${itemsFontSize}px` }}>
          {(items as any[])
            ...
            .map((item: any, idx: number) => {
              const unitPrice = 'unitPrice' in item ? item.unitPrice
                : (('total' in item && 'quantity' in item) ? item.total / item.quantity : 0);
              const totalPrice = 'totalPrice' in item ? item.totalPrice
