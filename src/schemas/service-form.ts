...
export const safetyInspectionSchema = z
  .object({
...
export const serviceFormSchema = z
  .object({
    ...
    initialStatus: z
      .enum(["Cotizacion", "Agendado", "En Taller", "Proveedor Externo", "Entregado", "Cancelado"])
      .optional(),
    ...
    appointmentDateTime: z.coerce.date().optional().nullable(),
    receptionDateTime: z.coerce.date().optional().nullable(),
    ...
    serviceAdvisorSignatureDataUrl: optionalUrl, // <- permite '' y valida URL si viene
    ...
    photoReports: z.array(photoReportSchema).optional().default([]),

    customerName: z.string().optional(),
    totalCost: z.coerce.number().optional(),

    // Sólo para abrir diálogos, no se valida
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.status === "Entregado") {
      ...
      // Validaciones por cada pago
      data.payments?.forEach((payment, index) => {
        // Requerir amount > 0 para cualquier pago en Entregado
        ...
        // Folio obligatorio para métodos no-efectivo
        if (
          (payment.method === "Tarjeta" ||
            payment.method === "Tarjeta MSI" ||
            payment.method === "Transferencia") &&
          (!payment.folio || payment.folio.trim() === "")
        ) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "El folio es obligatorio para este método de pago al entregar el servicio.",
            path: ["payments", index, "folio"], // <- RUTA CORRECTA
          });
        }
      });
    }
